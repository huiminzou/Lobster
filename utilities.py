...
def nearpoint_index(lat,lon,LAT,LON):#(lat1,lon1) is the point we want,(LAT,LON) is the point in database
        dists=[];points=[]
        for a in range(len(LAT)):
            for o in range(len(LON)):
                if lon-0.1<LON[o]<lon+0.1 and lat-0.1<LAT[a]<lat+0.1:
                    points.append((a,o))
                    dist=(LON[o]-lon)**2+(LAT[a]-lat)**2
                    dists.append(dist)
        if len(dists)>1:
...
def get_wind_ncep(starttime,endtime,lat,lon):
        #print starttime
        year=starttime.year
        url_input="""D:/projects/data/wind/ncep/"""
        ...
        for k in range(len(dtime)):
            times=starttime+td(hours=(dtime[k]-dtime[0]))
            dtimes.append(times)   
        return dtimes,u,v
